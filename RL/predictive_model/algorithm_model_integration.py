...
ckpt_dir    = "checkpoint_20"
ckpt_name   = 'policy_step_45000_seed_19.ckpt'
# set random seed
...
while True:
    ...
    print(f"goal_pos: {goal_pos}, target_pos: {target_qpos[:3]}")
    print(f"goal_rot: {goal_rot}, target_rot: {target_qpos[3:]}")
    print(f"rot_norm: {np.linalg.norm(target_qpos[3:])}")
    ...
    is_frozen = torch.all(torch.abs(hand_pos - prev_hand_pos) < 0.001, dim=1)
    ...
    # print(f"froze count: {froze_count}")
    # print(f"time_envs: {time_envs}")
    frozen_envs = (froze_count > 100) | (time_envs > max_timesteps)
        ...
        if torch.any(finished_envs):
            ...
            for i, finished in enumerate(finished_envs): 
                if finished & (len(unfinished_box_idxs[i]) == 0):
                    ...
                    if hand_pos_buffer[i].shape[0] > 10:
