...
class RgbHead(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cam: int, frame: int) -> Dict:
        rgb = self.rgb_head(x)
        rgb = torch.sigmoid(rgb)  # now in [0,1]
        return {f'rgb_cam_{cam+1}_{self.downsample_factor}_{frame}': rgb}
...
class StyleGanDecoder(nn.Module):
    ...
    def forward(self, w: torch.Tensor) -> Dict:
        batch_size, num_cam, frames, hidden_dim = w.shape
        ...
        for cam in range(num_cam):
            for frame in range(frames):
                ...
                output_4 = self.head_4(x, cam, frame)
                x = self.conv2(x, w_flat)
                output_2 = self.head_2(x, cam, frame)
                x = self.conv3(x, w_flat)
                output_1 = self.head_1(x, cam, frame)
