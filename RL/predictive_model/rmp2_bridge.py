"""
RMP2 Bridge for Isaac Sim + VR Teleoperation (Phase 1)
...
Architecture:
    Forward Pass:  q -> [eef_pos, cspace, collision_spheres]
...
RMP Leaves:
    1. TargetAttractor    - drives EEF toward VR-mapped goal pose
    2. CSpaceTarget       - attracts toward default/rest joint config
    3. JointLimit         - exponential barrier near joint limits
    4. JointVelocityCap   - dampens near max velocity
    5. JointDamping       - velocity-dependent jerk suppression
    6. ObstacleAvoidance  - per-obstacle repulsion
"""
...
class TargetAttractor:
    ...
    def eval(self, x, xd, **features):
        goal = features.get("goal", np.zeros(3))
        delta = np.asarray(goal, dtype=np.float64) - np.asarray(x, dtype=np.float64)
        delta_norm = np.linalg.norm(delta) + self.accel_norm_eps
        delta_hat = delta / delta_norm

        # PD-like acceleration
        accel = self.accel_p_gain * delta / delta_norm - self.accel_d_gain * np.asarray(xd, dtype=np.float64)
...
class CSpaceTarget:
    ...
    def __init__(self, metric_scalar=0.05, position_gain=20.0,
...
class ObstacleAvoidance:
    ...
    def eval(self, d, xd_coll, **features):
        ...
        base_metric = self.metric_scalar / (d / self.metric_exploder_std_dev + self.metric_exploder_eps)
        ...
        sig = 1.0 / (1.0 + np.exp(-xd_coll / self.damping_velocity_gate_length_scale))
        ...
        return np.array([[metric_val]], dtype=np.float64), np.array([accel_val], dtype=np.float64)
...
class JointLimit:
    ...
    def eval(self, dist, vel, **features):
        ...
        metric_before = self.metric_scalar / (dist / self.metric_length_scale + 1e-8)
    ...
    def eval(self, x, xd, **features):
        ...
        for i in range(n):
            if delta_vel[i] > 0:
                ...
                accel[i] = -self.damping_gain * delta_vel[i] * np.sign(xd[i])
...
class JointDamping:
    ...
    def __init__(self, accel_d_gain=30.0, metric_scalar=0.005, inertia=1e-4):
        self.accel_d_gain = accel_d_gain
        self.metric_scalar = metric_scalar
        self.inertia = inertia
...
class RMP2Solver:
    ...
    Usage:
        solver = RMP2Solver(n_joints=7, fk_fn=forward_kinematics)
        q_ddot = solver.solve(q, qd, goals=[vr_goal], obstacles=[obs_positions])
    ...
    def __init__(self, n_joints, fk_fn, collision_fn=None,
        ...
        Args:
            ...
            fk_fn(q) -> (eef_pos, J_ee): forward kinematics returning
                                         3-DoF position + 3xN Jacobian.
        ...
        self.default_q = (np.array(default_q, dtype=np.float64)
                          if default_q is not None else np.zeros(n_joints, dtype=np.float64))
        self.dt = dt

        # Fixed leaves in order: [target, cspace, jl, vel_cap, damping]
        ...
        # Names for debugging
        self._leaf_names = ["target", "cspace", "joint_limit", "vel_cap", "damping"]
    ...
    def forward_pass(self, q, qd, obstacles=None):
        ...
        J_ee = np.asarray(fk_out[1], dtype=np.float64)

        # 1. Target position RMP  (3-D Cartesian)
        J_target = J_ee[:3, :]
        xd_target = J_target @ qd
        results.append((eef_pos, xd_target, J_target))
        ...
        if self.joint_limits_low is not None and self.joint_limits_high is not None:
            ...
            dist_high = self.joint_limits_high - np.asarray(q, dtype=np.float64)
        ...
        # 4. Velocity cap  (uses xd, x ignored)
        results.append((np.zeros(n, dtype=np.float64), qd, np.eye(n, dtype=np.float64)))

        # 5. Damping  (uses xd, x ignored)
        results.append((np.zeros(n, dtype=np.float64), qd, np.eye(n, dtype=np.float64)))

        # 6. Collision — analytic Jacobian: ∂(‖eef−obs‖)/∂q = n̂^T · J_ee
        if self.collision_fn is not None and J_ee is not None and len(obstacles) > 0:
            ...
            if distances is not None:
                for obs_prim, d in zip(obstacles, distances):
                    ...
                    diff = np.asarray(eef_pos, dtype=np.float64) - np.asarray(obs_pos, dtype=np.float64)
                    d_norm = float(np.linalg.norm(diff)) + 1e-8
                    n_hat = diff / d_norm  # unit normal from obstacle → EEF
                    J_coll = (n_hat @ J_ee).reshape(1, n)   # (1, n) analytic
    ...
    # --------------------------------------------------------
    def solve(self, q, qd=None, goals=None, obstacles=None):
        """Return optimal joint acceleration q_ddot [n_joints]."""
        q = np.asarray(q, dtype=np.float64)
        qd = np.asarray(qd, dtype=np.float64) if qd is not None else np.zeros(self.n_joints, dtype=np.float64)
        obstacles = obstacles or []
        n = self.n_joints

        features = {
            "goal": np.asarray(goals[0], dtype=np.float64) if goals else np.zeros(3, dtype=np.float64),
            "obstacles": obstacles,
        }
        ...
        leaf_names = self._leaf_names + [f"obs_{i}" for i in range(n_obs)]
        ...
        for leaf, name, (x_leaf, xd_leaf, J_leaf) in zip(all_leaves, leaf_names, leaves_data):
            metric_val, accel_val = leaf.eval(x_leaf, xd_leaf, **features)

            # Pullback:  M_τ += J^T @ M @ J   ,  f_τ += J^T @ M @ a
