...
topic = "control/d0fee136-7315-49ed-9b53-a3973fe4128e-48c8gey"
...
def subscribe(client: mqtt_client):
    ...
    def on_message(client, userdata, msg):
        global vr_joints, vr_goal_pos, vr_goal_rot, grip_flag
...
def control_osc(dpose):
    ...
    u = torch.transpose(j_eef, 1, 2) @ m_eef @ (
        kp * dpose - kd * hand_vel[:, :6].unsqueeze(-1))
...
franka_num_dofs = gym.get_asset_dof_count(piper_asset)
...
for i in range(num_envs):
    ...
    # add franka
    franka_handle = gym.create_actor(env, piper_asset, franka_pose, "piper", i, 2)

    # set dof properties
    gym.set_actor_dof_properties(env, franka_handle, piper_dof_props)
    ...
    hand_handle = gym.find_actor_rigid_body_handle(env, franka_handle, "piper_hand")
    ...
    hand_idx = gym.find_actor_rigid_body_index(env, franka_handle, "piper_hand", gymapi.DOMAIN_SIM)
...
while not gym.query_viewer_has_closed(viewer):
    ...
    hand_vel = rb_states[hand_idxs, 6:]
    ...
    grasp_offset = 0.1 if controller == "ik" else 0.1
