...
from utils import slerp, get_observations, get_image, create_video_writer
import sys
...
def _create_model(args, **kwargs):
    ...
    print("RoboticDiffusionTransformerModel is instantiated")
    if pretrained is not None:
        # Check if it's a file or a directory with pretrained weights
        if os.path.isfile(pretrained):
            print("model policy file is being retrieved")
            model.load_pretrained_weights(pretrained)
        elif os.path.isdir(pretrained):
            # Reinitialize with pretrained path - this will be handled by get_policy()
            print("model policy directory is being retrieved")
            model.policy = model.get_policy(pretrained)
    return model
...
def update_observation_window(config, imgs, joint_state):
    # JPEG transformation
    # Align with training
...
with torch.inference_mode():
    while simulation_app.is_running():
        ...
        if captured_img:
            ...
            update_observation_window(config=config,imgs=captured_img, joint_state=current_q)
