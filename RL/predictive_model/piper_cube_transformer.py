...
custom_parameters = [
    ...
    {"name": "--seed",          "type": int,    "default": 7,       "help": "Set random seed for the simulation"},
]
...
for i in range(num_envs):
    ...
    # add piper
    piper_handle = gym.create_actor(env, piper_asset, franka_pose, "piper", i, 2)
...
dof_states      = gymtorch.wrap_tensor(_dof_states)  #torch.Size([24, 2])
...
try:
    os.makedirs("dataset/success_seed_" + str(args.seed), exist_ok=True)
    os.makedirs("dataset/failure_seed_" + str(args.seed), exist_ok=True)
    os.makedirs("videos/success_seed_" + str(args.seed), exist_ok=True)
    os.makedirs("videos/failure_seed_" + str(args.seed), exist_ok=True)
        ...
        if torch.any(finished_envs):
            ...
            for i, finished in enumerate(finished_envs): 
                if finished & (len(unfinished_box_idxs[i]) == 0):
                    ...
                    shutil.move(f"color_1_env_{i}.avi", f"videos/success_seed_{args.seed}/color_1_env_{i}_ep_{Eps_success[i]}.avi")
                    shutil.move(f"color_2_env_{i}.avi", f"videos/success_seed_{args.seed}/color_2_env_{i}_ep_{Eps_success[i]}.avi")
                    store_joints_states(i, Eps_success[i], dof_pos_buffer[i], True)
        ...
        if  torch.any(frozen_envs) & (len(unfinished_box_idxs[env_i]) < num_box - 1):
            for i, frozen in enumerate(frozen_envs): 
                if frozen:
                    ...
                    print(f"There is/are {len(unfinished_box_idxs[i])} left in the environment {i}")
                    shutil.move(f"color_1_env_{i}.avi", f"videos/failure_seed_{args.seed}/color_1_env_{i}_ep_{Eps_failure[i]}.avi")
                    shutil.move(f"color_2_env_{i}.avi", f"videos/failure_seed_{args.seed}/color_2_env_{i}_ep_{Eps_failure[i]}.avi")
                    store_joints_states(i, Eps_failure[i], dof_pos_buffer[i], False)
                    dof_pos_buffer[i] = torch.empty((0, 8))
                    Eps_failure[i] += 1
                    writers[i] = create_video_writer(i)
